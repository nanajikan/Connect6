...
class E6:
    ...
    def __init__(self, which_player, bs, c6, grphx):
        ...
        self.me_pri_1 = np.array([2, 6, 0, 1000]).reshape(4,1,1)
        self.op_pri_1 = np.array([1, 4, 10, 100]).reshape(4,1,1)
        self.me_pri_2 = np.array([2, 1, 6, 0, 1000]).reshape(5,1,1)
        self.op_pri_2 = np.array([1, 4, 6, 100, 100]).reshape(5,1,1)
    ...
    def tree(self):
        ...
        board_me_all = np.zeros((self.eval_pos**self.level, self.bs, self.bs))
        board_op_all = np.zeros((self.eval_pos**self.level, self.bs, self.bs))
        scores_moves = [(None, None)] * self.eval_pos**self.level
        null_el = np.zeros((self.bs,self.bs))
        board_me_all[0, :, :] = null_el + self.board_me
        board_op_all[0, :, :] = np.zeros((self.bs,self.bs)) + self.board_op
        # create tree
